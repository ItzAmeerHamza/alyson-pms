-- Aggregated screenshot storage + DeepSeek token usage per organization (for cost dashboards).
-- Requires ai_metadata.deepseek_usage from ai-screenshot-analyzer (populated on new analyses).
...
COMMENT ON FUNCTION public.get_organization_cost_insights(uuid) IS
  'Org-scoped screenshot bytes + DeepSeek token aggregates; callable by super_admin or org admin/org_admin.';
