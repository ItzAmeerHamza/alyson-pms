-- Fix cost insights scoping to use screenshots.organization_id.
-- This prevents zero totals when user profile org does not match historical screenshot ownership.
...
COMMENT ON FUNCTION public.get_organization_cost_insights(uuid) IS
  'Org-scoped screenshot bytes + DeepSeek token aggregates (scoped by screenshots.organization_id with fallback to users.organization_id).';
