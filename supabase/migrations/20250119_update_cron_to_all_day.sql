...
-- Step 1: Update/create the priority job to run all day
DO $$
DECLARE
  v_jobid integer;
BEGIN
  ...
  FROM cron.job
  WHERE jobname = 'ai-insights-worker-priority';

  IF v_jobid IS NULL THEN
    PERFORM cron.schedule(
      'ai-insights-worker-priority',
      '*/10 * * * *',
...
BEGIN
  ...
  PERFORM cron.schedule(
    'ai-insights-worker-business-hours',
    '*/5 9-17 * * 1-5',
    'SELECT public.run_ai_insights_priority();'
  );
END $$;
...
BEGIN
  ...
  FROM cron.job
  WHERE jobname = 'ai-insights-worker-daily';
...
BEGIN
  IF to_regclass('public.system_logs') IS NOT NULL THEN
    ...
    VALUES (
        'cron_update',
        'Updated AI analysis cron jobs to work all day',
        jsonb_build_object(
            'timestamp', now(),
